...
def extract_data_for_country(country_name):
    ...
    try:
        ...
        # Extract initial data on the first page
        st.write("Extracting data from the first page...")
        iata_values = driver.find_elements(By.XPATH, "//tr/td[2]")
        st.write(f"First page: Found {len(iata_values)} IATA rows.")

        # Extract Name, City, and Country values from the first page
        name_values = driver.find_elements(By.XPATH, "//tr/td[3]")
        ...
        while True:
            try:
                ...
                iata_values = driver.find_elements(By.XPATH, "//tr/td[2]")
                st.write(f"Page {page_count}: Found {len(iata_values)} IATA rows.")
...
while True:
    ...
    if st.sidebar.button(f"Extract Data for {country_name}", key=f"extract_{country_count}"):
        extract_data_for_country(country_name)
